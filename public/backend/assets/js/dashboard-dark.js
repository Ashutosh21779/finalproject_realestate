$(function() {
  ...
  // --- Agent Total Property Chart (Using static data like New Customers) ---
  if($('#agentTotalPropertyChart').length) {
    var optionsAgentTotalProp = {
      ...
      series: [{
        name: 'Properties', // Or keep empty like original
        // Using static data from original #customersChart for placeholder
        data: [3844, 3855, 3841, 3867, 3822, 3843, 3821, 3841, 3856, 3827, 3843] 
      }],
      stroke: {
        width: 2,
      ...
      colors: [colors.primary],
       // Remove xaxis for sparkline appearance
      xaxis: { categories: [] }, 
       // Remove yaxis for sparkline appearance
      yaxis: { min: 0 }, 
       // Minimal tooltip
       tooltip: { enabled: false } 
    };
    new ApexCharts(document.querySelector("#agentTotalPropertyChart"),optionsAgentTotalProp).render();
  }
  // --- End Agent Total Property Chart ---


  // --- Agent Message Chart (Using static data like Growth Chart) ---
  if($('#agentMessageChart').length) {
      var optionsAgentMsg = {
      chart: {
        type: "line", // Or bar like original #ordersChart if preferred
        height: 60,
        sparkline: {
          enabled: !0
        }
      ...
      series: [{
        name: 'Messages', // Or keep empty
         // Using static data from original #growthChart for placeholder
      ...
      colors: [colors.success], // Different color
       // Remove xaxis for sparkline appearance
      xaxis: { categories: [] },
       // Remove yaxis for sparkline appearance
      yaxis: { min: 0 }, 
    ...
    new ApexCharts(document.querySelector("#agentMessageChart"),optionsAgentMsg).render();
  }
  // --- End Agent Message Chart ---


  // --- Agent Property Status Chart (Modified from Orders Chart) ---
  if($('#agentPropertyStatusChart').length && typeof propertyStatusData !== 'undefined') {
    var optionsAgentStatus = {
      ...
      colors: [colors.primary, colors.success], // Colors for Rent/Buy
      series: [{
        name: 'Properties',
        // Use dynamic data: Ensure 'rent' and 'buy' keys exist or default to 0
        data: [propertyStatusData['rent'] ?? 0, propertyStatusData['buy'] ?? 0] 
      }],
      xaxis: {
        // Labels for the bars
        categories: ['Rent', 'Buy'] 
      ...
      yaxis: {
       ...
       tooltip: {
         enabled: true, // Enable tooltips to show exact counts
         y: {
           formatter: function (val) {
             return val + " Properties"; // Tooltip content
           }
         }
       }
    };
    new ApexCharts(document.querySelector("#agentPropertyStatusChart"),optionsAgentStatus).render();
  }
  // --- End Agent Property Status Chart ---
